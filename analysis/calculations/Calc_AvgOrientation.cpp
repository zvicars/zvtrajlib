#include "Calc_AvgOrientation.hpp"
#include "../../tools/stlmath.hpp"
#include "../../tools/pbcfunctions.hpp"
Calc_AvgOrientation::Calc_AvgOrientation(InputPack& input):Calculation_Histogram{input}
{
...
void Calc_AvgOrientation::calculate(){
  if(!doCalculate()) return;
  float sum = 0.0;
  double avgx=0.0, avgy=0.0, avgz=0.0;
  double nv=0.0;
  #pragma omp parallel for reduction(+:avgx) reduction(+:avgy) reduction(+:avgz) reduction(+:nv)
  ...
  avgy /= nv;
  avgz /= nv;
  Vec3<double> avgdir = {avgx, avgy, avgz};
  avgdir = avgdir;
  value_ = dot(avgdir, normal_); //scalar projection, normal_ is already normalized so no denominator
  if(doTimeseries || doHistogram){
    count_vec_.push_back(value_);
    vectors_.push_back(avgdir);
    step_vec_.push_back(box->frame_counter);
    time_vec_.push_back(box->time);
  }
  return;
}
std::string Calc_AvgOrientation::printConsoleReport(){
  std::stringstream ss;
  ss << "Name: " << name_ << ", Type: " << "Xi" << ", Output: " << value_ << std::endl;
  return ss.str(); 
}

void Calc_AvgOrientation::finalOutput(){
  ...
  if(doTimeseries){
    ...
    ofile << "#Output file for avgorientation calculation with name \'" << name_ << "\'\n";   
    ofile << "Timeseries: time (ps)     step     nv\n";
    for(std::size_t i = 0; i < count_vec_.size(); i++){
        ofile << time_vec_[i] << "     " << step_vec_[i] << "     " 
        << count_vec_[i] << "  " << vectors_[i][0] << "  " << vectors_[i][1] << "  " << vectors_[i][2] << "\n"; 
    }
    ofile.close();
  }
  return;
